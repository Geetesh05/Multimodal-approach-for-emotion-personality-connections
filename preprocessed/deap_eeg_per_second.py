...
for i in range(len(channel["Fp1"])):
  for j in ch_list:
    if str(channel["Fp1"][i])==str(j):
      key.append(i+1)
for n in os.listdir("/Volumes/TheJoker/amigos/mainfiles"):
  if not n.startswith("."):
   mat2 = scipy.io.loadmat("/Volumes/TheJoker/amigos/mainfiles/" + str(n))
   for i in range(len(mat2["data"])):
        eeg=[]
        yes = category(mat2["labels"][i])
        if not os.path.exists("/Volumes/TheJoker/deap/heatmaps/" + str(yes)):
            os.makedirs("/Volumes/TheJoker/deap/heatmaps/" + str(yes))
        for j in range(len(mat2["data"][i])):
              if j in key:
                  eeg.append(mat2["data"][i][j])
        theta=[]
        slow_alpha=[]
        alpha=[]
        beta=[]
        ...
        corrdinates = [(1, 3), (2, 0), (2, 2), (3, 1), (4, 0), (6, 0), (8, 3), (8, 5), (6, 8), (4, 8), (3, 7), (2, 6),
                     (2, 8), (1, 5)]
        for p in range(14):
                f, Pxx_den = signal.welch(eeg[p], fs=128.,scaling="spectrum")
        ...
        for m in range(len(bands)):
            ...
            plt.savefig("/Volumes/TheJoker/deap/heatmaps/"+str(yes)+"/frames%d.jpg" % count)